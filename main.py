...
CARD_OWNER = 'محمد کریم پور'
CONFIG_FILE = 'configs.txt'

bot = telebot.TeleBot(TOKEN)

plans = {
    '۱۰ گیگ - ۳۰ روزه 🚀': ('50,000 تومان', '10GB'),
    '۲۵ گیگ - ۳۰ روزه ⚡️': ('80,000 تومان', '25GB'),
    '۴۰ گیگ - ۳۰ روزه 🔥': ('105,000 تومان', '40GB'),
    '۷۵ گیگ - ۳۰ روزه 🧨': ('180,000 تومان', '75GB'),
    '۱۰۰ گیگ - ۳۰ روزه 💣': ('240,000 تومان', '100GB')
}
...
def send_payment_info(message):
    price, volume = plans[message.text]
    user_data[message.chat.id] = message.text  # ذخیره پلن انتخابی
    text = f"پلن انتخابی: *{message.text}*\nهزینه: *{price}*\n\nلطفاً مبلغ رو به شماره کارت زیر واریز کن و رسیدشو بفرست:\n\n`{CARD_NUMBER}`\nبه نام: *{CARD_OWNER}*"
    bot.send_message(message.chat.id, text, parse_mode='Markdown')

@bot.message_handler(content_types=['photo'])
def handle_photo(message):
    if message.photo:
        caption = user_data.get(message.chat.id, 'پلن نامشخص')
        bot.send_message(message.chat.id, 'رسید دریافت شد. منتظر تایید باش! ✅')
        bot.send_photo(ADMIN_ID, message.photo[-1].file_id, caption=f"رسید کاربر: {message.from_user.first_name} ({message.chat.id})\nپلن: {caption}", reply_markup=confirm_keyboard(message.chat.id))

# کیبورد تایید/رد برای ادمین
def confirm_keyboard(user_id):
    markup = types.InlineKeyboardMarkup()
    markup.add(types.InlineKeyboardButton("✅ تایید", callback_data=f"approve:{user_id}"))
    markup.add(types.InlineKeyboardButton("❌ رد", callback_data=f"reject:{user_id}"))
    return markup
